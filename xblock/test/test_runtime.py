...
class XBlockWithServices(XBlock):
    ...
    def student_view(self, _context):
        ...
        def assert_equals_text_type(str1, str2):
            """`str1` equals `str2`, and both are Unicode strings."""
            assert_equals(str1, str2)
            assert isinstance(str1, text_type)
            assert isinstance(str2, text_type)

        i18n = self.runtime.service(self, "i18n")

        # Python3 compatibility
        gettext = getattr(i18n, 'ugettext', i18n.gettext)
        ngettext = getattr(i18n, 'ungettext', i18n.ngettext)

        assert_equals_text_type(u"Welcome!", gettext("Welcome!"))

        assert_equals_text_type(u"Plural", ngettext("Singular", "Plural", 0))
        assert_equals_text_type(u"Singular", ngettext("Singular", "Plural", 1))
        assert_equals_text_type(u"Plural", ngettext("Singular", "Plural", 2))

        when = datetime(2013, 2, 14, 22, 30, 17)
        assert_equals_text_type(u"2013-02-14", i18n.strftime(when, "%Y-%m-%d"))
        assert_equals_text_type(u"Feb 14, 2013", i18n.strftime(when, "SHORT_DATE"))
        assert_equals_text_type(u"Thursday, February 14, 2013", i18n.strftime(when, "LONG_DATE"))
        assert_equals_text_type(u"Feb 14, 2013 at 22:30", i18n.strftime(when, "DATE_TIME"))
        assert_equals_text_type(u"10:30:17 PM", i18n.strftime(when, "TIME"))
